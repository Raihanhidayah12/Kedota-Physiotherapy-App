...
class _EditProfileScreenState extends State<EditProfileScreen>
  ...
  String get _displayPhone {
    ...
    if (raw.length > 6) {
      ...
      final maskedLen = (raw.length - prefix.length - suffix.length).clamp(3, 8);
  ...
  Future<void> _pickAndUploadPhoto() async {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _sheetOption(ctx, Icons.camera_alt_rounded,
                t(context, 'camera'), ImageSource.camera),
            const SizedBox(height: 8),
            _sheetOption(ctx, Icons.photo_library_rounded,
                t(context, 'gallery'), ImageSource.gallery),
            if (_profileImageUrl != null) ...[
              const SizedBox(height: 8),
              _sheetOption(ctx, Icons.delete_outline_rounded,
                  t(context, 'deletePhoto'), null, danger: true),
            ],
          ],
        ),
      ),
    ...
    try {
      ...
      final url     = await SupabaseAuthService()
          .uploadProfilePhoto(imageBytes: bytes, contentType: mime);
      if (!mounted) return;
      setState(() { _profileImageUrl = url; _isUploading = false; });
  ...
  Widget _sheetOption(BuildContext ctx, IconData icon, String label,
      ImageSource? src, {bool danger = false}) =>
      GestureDetector(
        ...
        child: Container(
          ...
          child: Row(children: [
            Icon(icon, size: 20,
                color: danger ? const Color(0xFFD94F45) : _c700),
  ...
  Future<void> _removePhoto() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _profileImageUrl = null; _isUploading = false; });
  ...
  Future<void> _pickDate() async {
    ...
    if (picked != null) setState(() { _birthDate = picked; _isDirty = true; });
  }
  ...
  String _monthName(int m) {
    ...
    final en = ['', 'January', 'February', 'March', 'April', 'May', 'June',
      'July', 'August', 'September', 'October', 'November', 'December'];
    final id = ['', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
      'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];
    return lang == AppLanguage.en ? en[m] : id[m];
  }

  void _snack(String msg) => ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(content: Text(msg), backgroundColor: _c700,
        behavior: SnackBarBehavior.floating,
                      ...
                      children: [
                        ...
                        _buildCard(children: [
                          _editableField(
                            ...
                            validator: (v) => (v == null || v.trim().isEmpty)
                                ? t(context, 'fullNameRequired') : null,
                          ),
                          _divider(),
                          _genderPicker(),
                          _divider(),
                        ...
                        _buildCard(children: [
                          _readOnlyField(
                            ...
                            onActionTap: () => setState(() => _hidePhone = !_hidePhone),
                          ...
                          _readOnlyField(
                            ...
                            value: _email?.isNotEmpty == true ? _email! : '-',
  ...
  Widget _buildAppBar() => SliverAppBar(
        ...
        title: Text(t(context, 'menuEditProfile'),
  ...
  Widget _sectionTitle(String text) => Padding(
        ...
        child: Text(text,
            style: const TextStyle(
                fontSize: 14, fontWeight: FontWeight.w700, color: _ink2)),
      );
  ...
  Widget _divider() => const Divider(
      height: 1, thickness: 1, color: Color(0xFFF0F4F4), indent: 16, endIndent: 16);
        ...
        child: Row(
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _ink3)),
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintText: hint,
                    hintStyle: const TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: _ink3),
                  ),
                ),
              ],
            ),
        ...
        child: Row(children: [
              ...
              children: [
                ...
                Text(value, 
                  ...
                  style: TextStyle(
                    ...
                    color: _ink,
                    letterSpacing: (onActionTap != null && _hidePhone) ? 1.0 : 0.0,
                  )),
              ],
            ),
          ),
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xFFF5F8F8), borderRadius: BorderRadius.circular(20)),
              child: Icon(actionIcon ?? Icons.lock_outline_rounded, size: 16, color: _ink3),
            ),
          ),
        ]),
      );
  ...
  Widget _genderPicker() => Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(t(context, 'gender'), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _ink3)),
            const SizedBox(height: 12),
            Row(
              children: _genders.map((g) {
                ...
                return Expanded(
                  child: GestureDetector(
                    onTap: () => setState(() { _gender = g; _isDirty = true; }),
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.calendar_month_rounded, size: 20, color: _c700),
            ),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(t(context, 'birthDate'), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _ink3)),
                  const SizedBox(height: 4),
                  Text(
                    _birthDate != null
                        ? _fmt(_birthDate!)
  ...
  Widget _changePinButton() => GestureDetector(
        onTap: () {
        ...
        child: Padding(
          ...
          child: Row(children: [
            Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(
                  color: const Color(0xFFFFF7F2), borderRadius: BorderRadius.circular(12)),
              child: const Icon(Icons.password_rounded, size: 20, color: Color(0xFFE87A3E)),
            ),
            const SizedBox(width: 16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(t(context, 'menuChangePin'),
                      style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _ink)),
                  const SizedBox(height: 4),
                  Text(t(context, 'menuChangePinSub'),
                      style: const TextStyle(fontSize: 12, color: _ink3)),
                ],
              ),
            ),
            const Icon(Icons.keyboard_arrow_right_rounded, size: 20, color: _ink3),
          ]),
        ),
      );
  ...
  Widget _saveButton() => Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: (_isDirty && !_isLoading)
              ? [BoxShadow(color: _c500.withValues(alpha: 0.3), blurRadius: 16, offset: const Offset(0, 8))]
              : null,
        ),
        child: ElevatedButton(
          ...
          child: _isLoading
              ...
              : Text(t(context, 'saveAndContinue'),
