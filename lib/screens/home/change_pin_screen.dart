...
class _ChangePinScreenState extends State<ChangePinScreen>
  ...
  void initState() {
    ...
    _enterCtrl.forward();
    _restoreNewPinCooldown();
  }

  Future<void> _restoreNewPinCooldown() async {
  ...
  Future<void> _checkNewPin(String pin) async {
    ...
    try {
      ...
      if (isSameAsOldPin) {
        final cooldown = await SupabaseAuthService().recordSameAsOldPinAttempt(
          phone: widget.phoneNumber,
        );
        if (!mounted) return;
        final shouldReturnToProfile = cooldown > 0;
        _sameAsOldPinAttempts++;
        final remainingAttempts = _maxAttempts - _sameAsOldPinAttempts;
        setState(() {
          _isLoading = false;
          _isPinError = false;
          _newPin = '';
          _newPinErrorMessage = shouldReturnToProfile
              ? null
              : t(
                  context,
                  'sameAsOldPinAttemptError',
                ).replaceAll('{attempts}', '$remainingAttempts');
        });
        if (shouldReturnToProfile) {
          await _showCooldownWarning(cooldown);
      ...
      if (mounted) _showError(msg: t(context, 'resetPinFailed'));
    }
  }

  Future<void> _showCooldownWarning(int seconds) async {
    await CustomBottomSheet.show(
      context,
      type: BottomSheetType.error,
      title: t(context, 'pinLimitTitle'),
      subtitle:
          '${t(context, 'sameAsOldPinError')}\n\n${t(context, 'wait30Seconds').replaceAll('{seconds}', '$seconds')}',
    ...
    try {
      ...
      if (valid) {
        ...
        if (_failedAttempts >= _maxAttempts) {
          ...
          _showError(
            msg: t(context, 'wrongPin').replaceAll('{attempts}', '$sisa'),
          );
        }
      }
    } catch (_) {
            ...
            children: [
              ...
              AnimatedSwitcher(
                ...
                child: _isLocked
                    ...
                    : const SizedBox(key: ValueKey('noerr'), height: 38),
              ),
            ],
          ),
    ...
    children: List.generate(6, (i) {
      ...
      return AnimatedContainer(
        ...
        decoration: BoxDecoration(
          shape: BoxShape.circle,
          color: _isPinError
              ? (filled ? const Color(0xFFD94F45) : Colors.transparent)
              : (filled ? _c500 : Colors.transparent),
          border: Border.all(
            color: _isPinError
          ...
          boxShadow: filled && !_isPinError
              ? [
                  BoxShadow(
                    color: _c500.withValues(alpha: 0.35),
                    blurRadius: 10,
    ...
    builder: (context, constraints) {
      ...
      return Padding(
        ...
        child: SizedBox(
          ...
          child: Column(
            children: [
              _numRow(['1', '2', '3'], btnSz, hGap),
              SizedBox(height: vGap),
              _numRow(['4', '5', '6'], btnSz, hGap),
              SizedBox(height: vGap),
              _numRow(['7', '8', '9'], btnSz, hGap),
              SizedBox(height: vGap),
              _numRow(['', '0', 'back'], btnSz, hGap),
            ],
          ),
        ),
      );
    ...
    if (val == 'back') {
      return GestureDetector(
        ...
        child: Container(
          ...
          child: Center(
            child: Icon(Icons.backspace_rounded, color: _c700, size: sz * 0.4),
          ),
        ),
      );
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          shape: BoxShape.circle,
          border: Border.all(
            color: _isLocked ? Colors.transparent : const Color(0xFFF0F4F4),
            width: 1.5,
          ),
          boxShadow: _isLocked
              ? null
